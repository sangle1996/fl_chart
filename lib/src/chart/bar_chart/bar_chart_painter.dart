...
class BarChartPainter extends AxisChartPainter<BarChartData> {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        if (barRod.backDrawRodData.show && barRod.backDrawRodData.y != 0) {
          if (barRod.backDrawRodData.y > 0) {
            // positive
            final bottom = getPixelY(max(data.minY, 0), viewSize, holder);
            final top = min(
                getPixelY(barRod.backDrawRodData.y, viewSize, holder),
                bottom - cornerHeight);
            ...
            // negative
            final top = getPixelY(min(data.maxY, 0), viewSize, holder);
            final bottom = max(
                getPixelY(barRod.backDrawRodData.y, viewSize, holder),
                top + cornerHeight);
        ...
        if (barRod.y != 0) {
          if (barRod.y > 0) {
            ...
            final bottom = getPixelY(max(data.minY, 0), viewSize, holder);
            final top = min(
                getPixelY(barRod.y, viewSize, holder), bottom - cornerHeight);
            ...
            final top = getPixelY(min(data.maxY, 0), viewSize, holder);
            final bottom =
                max(getPixelY(barRod.y, viewSize, holder), top + cornerHeight);
  ...
  void drawTouchTooltip(
    ...
    final barOffset = Offset(
      ...
      getPixelY(showOnRodData.y, viewSize, holder),
    );
  ...
  BarTouchedSpot? handleTouch(
    ...
    for (var i = 0; i < _groupBarsPosition!.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        if (isPositive) {
          barTopY =
              getPixelY(targetData.barGroups[i].barRods[j].y, viewSize, holder);
